...
async def Kaneki(_, message):
    if len(message.command) < 2:
        await message.reply_text("Enviction AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    Kaneki = text.replace(" ", "%20")
    m = await message.reply_text("Enviction Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=@Envictionbot&ownername=@Cyberhunt27&user=1"
        )
        chatbot = L["message"]
